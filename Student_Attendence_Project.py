...
def save_attendance_to_db(name,date,time):
    try:
        ...
        cursor = conn.cursor()
        check_query = "SELECT * FROM attendance_records WHERE student_name = %s AND date = %s"
        ...
        query = "INSERT INTO attendance_records (student_name, date, time) VALUES (%s, %s, %s)"
...
def save_data(name, frame, faces):
    student_path = os.path.join(BASE_DIR, name)
    ...
    for i in range(1,201):
        for (x,y,w,h) in faces:
            ...
            cv2.imwrite(os.path.join(student_path, f"{name}_{i}.jpg"), face_img)

        progress_bar.progress(i / 200)
        status_text.text(f"Saving Image: {i}/200")

    st.success(f"Success! {name} all data save '{student_path}'")
...
def train_system():
    ...
    if len(X) > 0:
        ...
        st.sidebar.success("Trained SuccessFully!")
...
def dashboard():
    conn = mysql.connector.connect(
            ...
            password="root",
            database="attendance_db")
    query = "SELECT  count(date) as date FROM attendance_records   group by date having CURDATE() = date "
...
if st.session_state.logged_in:
    ...
    if st.session_state.page =="Home":
        ...
        df = pd.read_sql("SELECT student_name,time,date From attendance_records ORDER BY id DESC LIMIT 5",mysql.connector.connect(
            host="localhost",
            user="root",
            password="root",
            database="attendance_db"
        ...
        if camera_img is not None and name_input:
            ...
            if len(faces) > 0:
                ...
                if st.button("Generate 200 Sample Now"):
        ...
        if pca is None or model is None:
            ...
            test_img = st.camera_input("Scanning For Attendance")
                ...
                if len(faces) > 0:
                    for (x,y,w,h) in faces:
                        ...
                        if save_attendance_to_db(name, current_date, current_time):
                            st.success(f"Attendance marked for {name} in MYSQL!")
